...
namespace lab34.Services
{
    public unsafe class Rasterizer
    {
        ...
        public void UpdateCamera(Vector3 cameraPos) => _cameraPos = cameraPos;
            ...
            for (int y = iy1; y <= iy3; y++)
            {
                ...
                for (int x = ixStart; x <= ixEnd; x++)
                {
                    ...
                    if (z < _zBuffer[idx])
                    {
                        ...
                        buffer[y * _width + x] = LightingHelper.ComputeColor(norm, pos, _lightPos, _lightColor, _objectColor, _cameraPos);
                    }
                }
            }
        }
